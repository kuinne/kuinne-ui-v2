...
module.exports = defineConfig({
  ...
  // 将浏览器 API、 ES API 和 Node API 看作全局变量，不会被特定的规则（如 no-under）限制
  env: {
    browser: true,
    es2022: true,
    node: true,
  ...
  // 设置自定义全局变量，不会被特定的规则（如 no-under）限制
  globals: {
  ...
  parserOptions: {
    ...
    // 通过 tsconfig 文件确定解析范围，这里需要决定路径，否则子模块中 eslint 会出现异常
    ...
    // TypesScript 解析器也要负责 vue 文件的 <script>
