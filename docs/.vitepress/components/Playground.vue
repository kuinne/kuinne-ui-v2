...
async function fetchUiVersions() {
  const res = await fetch('https://data.jsdelivr.com/v1/package/npm/@kuinne/ui');
  const { versions } = (await res.json()) as { versions: string[] };
...
function setUiVersion(version: string) {
  ...
  store.setImportMap({
    imports: {
      '@kuinne/ui': `https://fastly.jsdelivr.net/npm/@kuinne/ui@${version}/dist/kuinne-ui.full.min.mjs`,
    },
  });
  // 加载组件库的全量样式
  previewOptions.headHTML = `<link rel="stylesheet" href="https://fastly.jsdelivr.net/npm/@kuinne/ui@${version}/dist/style/index.css">`;
}
  ...
  <div v-if="isMounted">
    ...
    <Teleport to=".VPNavBarSearch">
      <div class="flex items-center text-14px">
        <label class="playground-label">KuinneUI: </label>
