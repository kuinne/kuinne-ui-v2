...
import { kuinneuiPreset, KuinneuiPresetOptions } from '../../styles/src/unoPreset';
import { generateConfig } from './common';
import { GenerateConfigOptions } from '../src';
import { absCwd, relCwd } from '../src/utils';
...
export interface GenerateVueConfigOptions extends GenerateConfigOptions {
  ...
  /** 传递给组件库 UnoCSS 预设的选项 */
  presetKuinneuiOptions?: KuinneuiPresetOptions
}
...
export async function generateVueConfig(
  ...
  const { pluginUno = true, presetKuinneuiOptions, presetUnoOptions } = customOptions || {};
    ...
    plugins: [
      pluginUno ? unocss({
        ...
        presets: [
          ...
          kuinneuiPreset(presetKuinneuiOptions),
        ],
        transformers: [
