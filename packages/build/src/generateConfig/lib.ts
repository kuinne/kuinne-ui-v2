...
import {
  absCwd, camelCase, kebabCase, relCwd,
} from '../utils';
...
export function getLib(packageJson: PackageJson = {}, options: GenerateConfigOptions = {}): Pick<BuildOptions, 'lib' | 'minify' | 'sourcemap' | 'outDir' | 'emptyOutDir'> {
  const {
    entry, outDir, mode, fileName,
  } = getOptions(options);

  // 文件名称，默认取 package.json 的 name 字段转换成 kebab-case： @kuinne/build => kuinne-build
  ...
  return {
    lib: libOptions,
    // full-min 模式下全量构建，需要混淆代码，生产 sourcemap 文件，且不清空产物目录
