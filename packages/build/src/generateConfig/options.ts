...
import type { GenerateConfigPluginOptions } from './plugins';

/** 自定义构建选项 */
export interface GenerateConfigOptions extends GenerateConfigPluginOptions {
  /**
   * 代码入口
   ...
   * 生成的文件名称
   *
   * 默认情况下取 package 包名，转换为 kebab-case，如: @kuinne/request -> kuinne-request
   *
   * 当产物为 umd 格式时，驼峰化后的 fileName 会作为全局变量名，如： kuinne-request -> kuinneRequest
   */
  fileName?: string

  /**
   * 打包模式
   * - package - 常规构建。会将所有依赖外部化处理，打包出适用于构建场景的 `es`、`umd` 格式产物。并在构建结束后将产物路径写回入 package.json 的入口字段中。
   * - full - 全量构建。大部分依赖都不做外部化处理，打包出适用于非构建场景的的 `umd`格式产物。不参与 d.ts 的移动；不参与构建完成后的产物路径回写
  ...
  /**
   * 完成构建后，准备回写 package.json 文件前其对象进行更改的钩子
   *
   * 必须在 mode 为 packages 时生效
   */
