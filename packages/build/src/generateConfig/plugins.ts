 ...
 */
export function getPresetPlugins(options: GenerateConfigPluginOptions = {}) {
...
/**
 * 处理单个预设插件
 * @param options 语塞插件相关配置选项
 * @param key key 目标选项名称
 * @param plugin plugin 对应的插件函数
 * @param defaultOptions 插件默认选项
 */
export function getPresetPlugin<K extends keyof GenerateConfigPluginOptions>(
  options: GenerateConfigPluginOptions,
  key: K,
  plugin: (...args: any[]) => PluginOption,
  defaultOptions?: GenerateConfigPluginOptions[K],
) {
