...
export type DefaultPrefix = 'ke-';

/** 默认情况下，生成 CSS 变量时增加的前缀 */
export const DEFAULT_PREFIX: DefaultPrefix = 'ke-';
...
export function generateCssVars<
  ...
  Object.entries(origin).forEach(([key, value]) => {
    ...
    // 对指定键值的变量生成色阶
    if (valueToRgba && colorLevelsEnableKeys.includes(key as K)) {
...
/**
 * 将 css 变量对象转化为 css 样式字符串
 * @param cssVars CSS 变量对象
 * @param selector selector 应用样式的选择器
 */
