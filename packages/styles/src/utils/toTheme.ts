import {
  DEFAULT_PREFIX, DefaultPrefix, GenerateCssVarsOptions, getCssVar,
} from './cssVars';
/**
 * 主题生成选项
...
export function toTheme<
  ...
  keys.forEach((key) => {
    // 主题必须符合类似 rgb(var(--ke-color-primary)) 的格式，这样 UnoCSS 能生成的原子类既能支持 CSS 变量，又能支持透明度修改
    result[key] = `rgb(${getCssVar(`${type}-${key}`, prefix)})`;

    // 处理色阶主题
    if (type === 'color' && colorLevelsEnableKeys.includes(`${type}-${key}` as K)) {
