...
import { absCwd, relCwd } from '../build/src/utils';
...
export default defineConfig(({ mode }) => {
  if (mode === 'unocss') {
    ...
    return generateConfig({
      ...
      dts: '',
      // 指定 exports 字段，将构建产物的相对路径写入 package.json 中的 exports['./preset]
      exports: './preset',
    });
  }

  return generateVueConfig({
    // 在 package.json 的 exports['./style.css] 为样式文件的入口
    ...
    presetKuinneuiOptions: {
      // 基础主题样式的 CSS 由 UnoCSS 生成，需要正确指定 kuinneuiPreset 的模块
      include: ['theme'],
    },
  }, {
    build: {
